<template>
  <div id="singup">
    ...
    <div class="new" v-on:click="backSingin">Back to Sign Up</div>
  </div>
</template>
...
<script>
  module.exports = {
    ...
    methods:{
      backSingin: function(){
        this.getMain().isOpen = 'on';
      },
      submit: function(){
        var msg = [];
        //this.username,this.email,this.password,this.reenterPassword
        if(this.username == ''){
          msg.push('Username');
        }
        if(this.email == ''){
        ...
        if(this.reenterPassword == ''){
          msg.push('ReenterPassword');
        }

        if(msg.length != 0){
          msg = ('Please Enter values in '+msg.join(', ') +'.');
        }else{
          msg = '';
        }

        if(this.reenterPassword != this.password && this.password != ''){
          if(msg != ''){
            msg += '\nand';
          }
